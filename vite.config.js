...
function resolveHtmlCandidate(rootAbs, pathname) {
  ...
  if (p.endsWith(".html")) {
    const f = path.join(rootAbs, p.replace(/^\//, ""));
    return fs.existsSync(f) ? f : null;
  }
  if (p.endsWith("/")) {
    const f = path.join(rootAbs, p.replace(/^\//, ""), "index.html");
    return fs.existsSync(f) ? f : null;
  }
  return null;
}
...
function addDevNotFound(server) {
  ...
  server.middlewares.use((req, res, next) => {
    ...
    server.transformIndexHtml("/404.html", raw)
      .then(html => {
        res.statusCode = 404;
...
function handlebarsPre(opts) {
  const p = handlebars(opts);
  return { ...p, enforce: "pre" };
}
